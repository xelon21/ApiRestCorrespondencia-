...
const poolL = mysql.createPool(database2);
...
/** GetConnection de la base de datos Login */
poolL.getConnection((err, conneccion) => {
    if(err){
        if(err.code === 'PROTOCOL_CONNECTION_LOST') {
            console.error('LA CONECCION DE LA BASE DE DATOS FUE CERRADA');
        }
